...
A daily snapshot dashboard for the tech/geek community — four tabs, each a card grid you can click into for details, refreshed once a day:
...
1. A GitHub Actions workflow (`.github/workflows/scrape.yml`) runs daily at **12:00 UTC** and pulls from four sources: the [HN Algolia API](https://hn.algolia.com/api) (front page + comments + the current "Who is hiring?" thread), `github.com/trending` (HTML, no official API) enriched with the GitHub REST API for homepage/topics, and the [Dev.to API](https://dev.to/api) filtered to `#gamedev`.
