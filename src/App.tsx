...
import { PHModal } from './components/PHModal';
...
export default function App() {
  ...
  const [selectedLaunch, setSelectedLaunch] = useState<PHLaunch | null>(null);
  ...
  const productHuntLaunches = data.productHuntLaunches ?? [];
    ...
    <div className="app">
      ...
      <main className="app__main">
        ...
        {activeTab === 'producthunt' && (
          <section className="panel">
            <h2 className="panel__heading">Top Products Launching Today</h2>
            <div className="story-grid">
              {productHuntLaunches.map((launch) => (
                <PHCard key={launch.id} launch={launch} onOpen={setSelectedLaunch} />
              ))}
            </div>
            {productHuntLaunches.length === 0 && (
              <p className="comments__empty">No Product Hunt data for this day.</p>
            )}
          </section>
        )}
      </main>
      <footer className="app__footer">
        ...
        Forem/Dev.to, Stack Exchange, or Product Hunt.
      </footer>
      ...
      {selectedLaunch && (
        <PHModal launch={selectedLaunch} onClose={() => setSelectedLaunch(null)} />
      )}
    </div>
  );
}
